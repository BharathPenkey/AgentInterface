            ...
            <div class="message ${msg.type}-message ${msg.isTyping ? "typing-indicator" : ""}">
              ...
              <div class="messageMeta">
                ...
                ${msg.type === "user" && msg.caller 
                  ...
                  : ""}
               ${msg.type ==="bot"
                          ? `<img src="./store.png" alt="artifactStoreIcon" class="artifactStoreIcon" data-index="${index}"  data-chat-id="${msg.chatId || ""}"/>` : ""}  
...
if (icon) {
  ...
  addMessage(
    ...
    this.chatArray.push({
      ...
      msg,
  entityName,agentData,sourceAgent,artifactObj,resAgent,
  triggeredByUser: type === "bot" && this.awaitingBotResponse, // Mark bot messages triggered by user input

    });

      // Reset awaitingBotResponse flag after bot response
  if (type == "bot") {
    ...
    if (type == "bot") {
      this.playLastBotMessage();
    }
  }

  playLastBotMessage() {
    const lastMessage = this.chatArray[this.chatArray.length - 1];
    console.log("Attempting to play last bot message:", lastMessage);
  
    // Ensure the last message is a bot response and was triggered by user input
    if (lastMessage && lastMessage.type === "bot" && lastMessage.triggeredByUser) {
      console.log("Playing message:", lastMessage.content);
      this.toggleMessagePlayback(lastMessage.content);
  
      // Reset the `triggeredByUser` flag after playback
      lastMessage.triggeredByUser = false;
    } else {
      console.log("No eligible message to play automatically.");
    }
  }
...
async handleFavIconClick(chatId) {
  ...
  async artifactStore(chatId) {
    console.log("chatId:", chatId);
  
    const apiEndpoint = "https://ig.gov-cloud.ai/mobius-gpt-service/artifact";
    const apiToken = this.apiToken; // Replace with your actual token
    const uniqueName = `artifact_${chatId}_${Date.now()}`;
 
    const requestData = {
      promptId: chatId,
      aritfact_type: "gernal",
      artifact_location: "plateform",
      name: uniqueName,
    };
    ...
    try {
      const response = await fetch(apiEndpoint, {
        ...
        headers: {
          "Content-Type": "application/json",
          token: apiToken, // Note: Token header matches `curl`
        },
      ...
      const data = await response.json();
      console.log("Artifact store response:", data);
    } catch (error) {
      console.error("Error in artifact store action:", error);
    }
  }
  ...
  addEventListeners() {
    ...
    const favIcons = this.querySelectorAll(".favIcon");
    const audioIcon = this.querySelector("#audio");
    const artifactIcons = this.querySelectorAll(".artifactStoreIcon"); // Use querySelectorAll
    ...
    artifactIcons.forEach((icon) => { // Use the corrected NodeList here
      icon.addEventListener("click", async (event) => {
        const chatId = event.target.dataset.chatId;
        if (chatId) {
          console.log("Chat ID for this user message:", chatId);
          event.target.src = "./storeSelected.png"; // Change the image to indicate selection
          await this.artifactStore(chatId);
        } else {
          console.log("No chatId associated with this user message.");
        }
      });
  ...
  async handleSendMessage(userMessage) {
    console.log("User message received:", userMessage);    // this.addMessage(userMessage, "user", `temp-${Date.now()}`);
    const tempChatId = `temp-${Date.now()}`; // Temporary ID for immediate rendering
    this.addMessage(userMessage, "user", tempChatId);
     // Set the flag to await a bot response
  this.awaitingBotResponse = true;
    ...
    if (agentReasoning) {
      ...
      console.log("Adding bot response:", agentReasoning);
      this.addMessage(agentReasoning, "bot", chatId);
    }
    else {
      console.error("No agent reasoning received for bot response.");
    }
    // this.addMessage(botResponse, "bot");
  }
}
